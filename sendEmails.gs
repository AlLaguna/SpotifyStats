...
function sendWeeklyTopTracksEmail() {
    ...
    topArtists = getTopArtists(5, 'semana');
    ...
    top10.forEach((track, index) => {
      ...
      if(index < 5){
        ...
        const imageUrl = getCover(top10[index].albumId, "albums");
...
function sendMonthlyTopTracksEmail() {
    ...
    const today = new Date();
    topArtists = getTopArtists(5, 'mes');
     topArtists.forEach((artistData, index) => {
      emailBody += `<h2>${index + 1}. ${artistData.artist}: ${artistData.plays} reproducciones</h2>`;
    });
    ...
    top50.forEach((track, index) => {
      ...
      if(index < 5){
        ...
        const imageUrl = getCover(top50[index].albumId, "albums");
        const imageBlob = UrlFetchApp.fetch(imageUrl).getBlob().setName(`cover_${index}`);
        inlineImages[`cover_${index}`] = imageBlob;
      }
    ...
    //createMonthlyPlaylist(topTracks, 'Este mes has escuchado ' + time.hours + " horas y " + time.minutes + ' minutos');

    const playlistName = `${monthNames[currentDate.getMonth()-1]} ${currentDate.getFullYear()}`;
    createPlaylist(topTracks, playlistName, 'Este mes has escuchado ' + time.hours + " horas y " + time.minutes + ' minutos');
}

function getCover(id, type) {
  //type: 'albums', 'artists'
  //Comprueba si esta en la caché
  const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName("covers");

  // Get all data from the covers sheet
  const data = sheet.getDataRange().getValues();
  
  // Check existing entries
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === id) { // Assuming albumId is in column A
    ...
    Logger.log(`https://api.spotify.com/v1/${type}/${id}`);
    // Hacer una solicitud GET a la API de Spotify para obtener los detalles del 
      let response = UrlFetchApp.fetch(`https://api.spotify.com/v1/${type}/${id}`, {
        headers: {
    ...
    const coverUrl = responseData.images[0].url;
  
    const name = responseData.name;

    // Add new entry to the sheet
    sheet.appendRow([id, coverUrl, name]);
  ...
  function sendYearlyTopTracksEmail() {
      ...
      const today = new Date();
      topArtists = getTopArtists(10, 'anio');
      topArtists.forEach((artistData, index) => {
        emailBody += `<h2>${index + 1}. ${artistData.artist}: ${artistData.plays} reproducciones</h2>`;
      });
    ...
    */
      ...
      const playlistName = `Top ${currentDate.getFullYear()}`;
      createPlaylist(topTracks, playlistName, 'Este año has escuchado ' + time.hours + " horas, " + time.hours/24 + ' días!');
  }
